    ...
    sudoRule = {
      ...
      commands =
        builtins.map (cmd: {
          ...
          "/run/current-system/sw/bin/env ^NIXOS_INSTALL_BOOTLOADER(=(0|1))? /run/current-system/sw/bin/systemd-run -E LOCALE_ARCHIVE -E NIXOS_INSTALL_BOOTLOADER(=(0|1)?) --collect --no-ask-password --pipe --quiet (--same-dir )?--service-type=exec --unit=nixos-rebuild-switch-to-configuration /nix/store/([a-z0-9]+)-nixos-system-${config.networking.hostName}-([0-9.a-z]+)/bin/switch-to-configuration (switch|boot|test|dry-activate)$"
          "/run/current-system/sw/bin/env ^NIXOS_INSTALL_BOOTLOADER(=(0|1))? systemd-run -E LOCALE_ARCHIVE -E NIXOS_INSTALL_BOOTLOADER(=(0|1)?)? --collect --no-ask-password --pipe --quiet (--same-dir )?--service-type=exec --unit=nixos-rebuild-switch-to-configuration /nix/store/([a-z0-9]+)-nixos-system-${config.networking.hostName}-([0-9.a-z]+)/bin/switch-to-configuration (switch|boot|test|dry-activate)$"
